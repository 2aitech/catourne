      ...
      <div className="relative z-10 flex flex-1 flex-col justify-center px-4 md:px-8">
          ...
          {visibleIndices.map((offset) => {
            ...
            return (
              <motion.div
                key={absIndex}
                className={cn(
                  "absolute aspect-[3/4] w-[260px] md:w-[300px] rounded-2xl border-t border-white/20 bg-noir-900 shadow-2xl transition-shadow duration-300",
                  isCenter ? "z-20 shadow-gold-500/10" : "z-10"
                )}
                initial={false}
                animate={{
                  x: xOffset,
        ...
        <div className="mx-auto mt-12 flex w-full max-w-4xl flex-col items-center justify-between gap-6 md:flex-row pointer-events-auto">
          <div className="flex flex-1 flex-col items-center text-center md:items-start md:text-left h-32 justify-center">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <h2 className="text-3xl font-bold tracking-tight md:text-4xl text-cream-50 font-serif">
                  {activeItem.title}
                </h2>
                {activeItem.description && (
                  <p className="max-w-md text-cream-500">
                    {activeItem.description}
                  </p>
                )}
              </motion.div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-1 rounded-full bg-noir-900/80 p-1 ring-1 ring-white/10 backdrop-blur-md">
              <button
                onClick={handlePrev}
                className="rounded-full p-3 text-cream-500 transition hover:bg-white/10 hover:text-cream-100 active:scale-95"
                aria-label="Previous"
              >
                <ChevronLeft className="h-5 w-5" />
              </button>
              <span className="min-w-[40px] text-center text-xs font-mono text-cream-600">
                {activeIndex + 1} / {count}
              </span>
              <button
                onClick={handleNext}
            ...
            {activeItem.href && (
              <Link
                to={activeItem.href}
                className="group flex items-center gap-2 rounded-full bg-gold-500 px-5 py-3 text-sm font-semibold text-noir-950 transition-transform hover:scale-105 active:scale-95"
              >
                Voir tout
